...
function getProgramNameFromURL(url: string){
...
async function download(url: string){
    ...
    await checkPrerequisitesPromises().then((results) => { //instead of making the function actually print out if it fails or not, we do this because otherwise we would somehow need to make that other function return a promise, and await inside a function that's called by a function that isn't await doesn't work
        ...
        results.forEach((result) => {
            if(result){ //meaning command failed
...
async function compile(programName: any, ignoreScrypty = 0){
    ...
    let methods: string | any[] = []; //the methods of compilation, the earlier in the array, the better method of compilation, we still ask the user, but inform them which one is the better option

    let scryptyFile = "none";
    let scrypty : ScryptyFile = blankScryptyFile;
    ...
    if(s.findIfScrypty(files) && ignoreScrypty == 0){
            console.log("Found Scrypty file!");
            s.log("Found scrypty file");
            scryptyFile = folder + "\\" + files.find((element: string) => { return element.endsWith(".scrypty");} );
    ...
    if(validScrypty == 0){
        ...
        file = await findMethods(folder, programName, methods); //no idea why, but methods is global, so we don't even need to return that
    }
...
function getDirectories(src: string) : Promise<Array<string>>{ 
    return new Promise(resolve =>{ //to make it valid with await
        glob(src + '/**/*', (error: any, res: string[] | PromiseLike<string[]>) => resolve(res));
    });
}



async function findMethods(folder: string, programName: any, methods: any[]) : Promise<Map<string, string>>{
...
function findIfGradle(folder: string){
    return fs.existsSync(folder + "/build.gradle");
}

async function findIfCpp(folder: string, programName: string){
    ...
    return new Promise((resolve) => {
        ...
        resolve(file); //why is this a promise???? idfk 
    });
}

async function findVSMethod(folder: any, programName: string){ //todo make it so if there's only one sln, just continue
    ...
    let res = await getDirectories(__dirname + "\\" + programName); //uhh this returns all files in the folder, which takes a while, but compiling takes longer so the user will have to wait :)
        ...
        let preferredSlns = slns.filter((element) => { return (element.toLowerCase().substr(element.lastIndexOf("/")).indexOf(programName) != -1) ?  element : "" }); //the preferred sln is the slns in the array with the programName in the file name
        let notPreferredSlns = slns.filter((element) => { return (element.toLowerCase().substr(element.lastIndexOf("/")).indexOf(programName) == -1) ?  element : "" }); //to filter out the rest
        ...
        while(!validNum){
            ...
            r = prompts("Type in '+' for more options:");
            ...
            if(parseInt(r) || r == "0"){
                ...
                slns = res.filter((element) => { return element.endsWith(".sln") || element.endsWith(".slnf") || element.endsWith(".vcxproj") || element.endsWith(".csproj"); });
                ...
                s.log("Preferred Solutions/vcxproj/csproj/slnf: " + preferredSlns, 2);
                s.log("Not Preferred Solutions/vcxproj/csproj/slnf: " + notPreferredSlns, 2);
                if(preferredSlns.length != 0){
                    console.log(colorText(_green, _bright + "These solutions are the better option to choose from (because they have the repo's name in the file name)"));
                    for(let i = 0; i < preferredSlns.length; i++){
                        console.log(colorText(_magenta, _bright + "[" + (i+1) + "]")  + " " + preferredSlns[i]);
...
async function findIfCMake(folder: string){
...
async function compileByMethod(method: any, scrypty: ScryptyFile, folder: string, programName: string, file = new Map()){
    switch(method){
        case "singleg++": 
            if(scrypty){
                compileSingleGPP(folder, getScryptyOS(scrypty)., programName); break;
            }
            else {
        ...
        case "gradle":
            ...
            break;
        case "custom":
            compileCustom(scrypty, programName);
            break;
    }
    console.log("Repository installed! Program is found in: " + folder + ". Run this program with `scrypty run " + programName + "`");
}

async function compileSingleGPP(folder: string, file: string, programName: string){
    if(file == "none" || file === undefined || file == "2"){ 
        if(fs.existsSync(folder + "\\main.cpp") || fs.existsSync(folder + "\\" + programName + ".cpp") || fs.existsSync(folder + "\\index.cpp")){
            ...
            if(files.length > 1){
                ...
                while(!validNum){
                    r = prompts("Choose the .cpp file by typing in the number next to the file: ");
    ...
    if(file == "none" || file === undefined || file == "2"){ //if we still have no file selected
        let files = await getDirectories(__dirname + "\\" + programName);
    ...
    while(!validNum){
        ...
        let r = prompts(colorText(_green, _bright + "Choose the C++ version to use (if unsure, try C++17, and if that doesn't work, go down the chain. You can try using C++20, but it's so new, some compilers might not support it): "));

        if(parseInt(r) || r == "0"){
            if(parseInt(r) > 0 && parseInt(r) <= 5){
                switch(r){
    ...
    exec("g++ --std=" + cppVer + " " + folder + "\\" + file + " -o " + folder + "\\" + file.substr(0, file.lastIndexOf(".")) + (os.platform() == "win32" ? ".exe" : ""), (error: { message: any; }, stdout: any, stderr: any) => {
        if (error) {
...
function compileSingleGCC(folder: string, file: string){ //todo, test it
    console.log("Compiling file " + file + "...");
    s.log("Compiling file " + file + "...");
    exec("gcc " + folder + "\\" + file + " -o " + folder + "\\" + file.substr(0, file.lastIndexOf(".")) + (os.platform() == "win32" ? ".exe" : ""), (error: { message: any; }, stdout: any, stderr: any) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
...
function compileSingleGo(folder: string, file: string){ //todo, test it
    console.log("Compiling file " + file + "...");
    s.log("Compiling file " + file + "...");
    exec("cd " + folder  + " && go build " + folder + "\\" + file, (error: { message: any; }, stdout: any, stderr: any) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
...
function compileVSSolution(folder: any, file: string, programName: any){
    ...
    */
    ...
    fs.readFile(file, 'utf8', (err: any, data: string) => {
        ...
        sln = sln.filter((element: string | string[]) => { return element.indexOf("|") != -1});
        ...
        console.log(colorText(_green, "Configuration Platforms:")); //todo, if there are no config platforms (like with slnf), just skip
        ...
        while(!validNum){
            ...
            let r = prompts("Choose a configuration platform to compile: ");

            //geniunely no idea why this works, does prompt make it an integer already? if so, i know how this works
            if(parseInt(r) || r == "0"){
                if(parseInt(r) > 0 && parseInt(r) <= sln.length){
                    config = configs[parseInt(r)-1];
                    platform = platforms[parseInt(r)-1];
                    validNum = true;
        ...
        while(!validNum){
            ...
            let r = prompts("Select your Visual Studio version (if you just downloaded it, then you probably have 2022): ");

            if(parseInt(r) || r == "0"){
                if(parseInt(r) > 0 && parseInt(r) <= 3){
                    switch(r){
        ...
        while(!validNum){
            ...
            let r = prompts("Select the preferred Windows 10 SDK: ");

            if(parseInt(r) || r == "0"){
                if(parseInt(r) > 0 && parseInt(r) <= 6){
                    switch(r){
        ...
        exec("msbuild -t:restore" + sdk + " -p:RestorePackagesConfig=true " + pt + cp + " " + file, {maxBuffer: 1024 * 4000}, (error: { message: string; }, stdout: string, stderr: string) => { //restore nuget pacakges (if needed)
            if (stderr) {
                console.log(`${stderr}`);
            ...
            exec("msbuild " + file + sdk + pt + cp, {maxBuffer: 1024 * 4000}, (error: {message: any}, stdout: string, stderr: string) => {
                if (stderr) {
                    console.log(`${stderr}`);
...
function compileCmake(folder: string, programName: any){
    //still so much to do
    //cmake can have build types, similar to configurations on vs
    ...
    let option;
    while(!validNum){
       let r = prompts(colorText(_cyan, "Choose the CMake command to do. If this the first time you're seeing this prompt while installing this repo, then press 1 (setup build environment): "));
    ...
    switch(option){
        case "1": exec("cd " + folder + " && cmake -S ./ -B ./build", (error: {message: any}, stdout: string, stderr: string) => {
            if (error) {
                console.log(`error: ${error.message}`);
        ...
        case "2": exec("cd " + folder + " && cmake --build ./", (error: {message: any}, stdout: string, stderr: string) => {
            if (error) {
                console.log(`error: ${error.message}`);
        ...
        case "3": exec("cd " + folder + " && cmake --install ./", (error: {message: any}, stdout: string, stderr: string) => {
            if (error) {
...
function compileGradle(folder: string){
    s.log("Gradle Compile");
    exec("cd " + folder + " && gradlew build", (error: {message: any}, stdout: string, stderr: string) => {
        if (error) {
            console.log(`error: ${error.message}`);
...
async function compileCustom(scrypty: any, programName: any){
    ...
    while(r != ("y" || "n")){
        r = prompts("Y (Continue) | N (Don't continue) | C (Check commands)");
        r = r.toLowerCase();
        switch(r){
            case "y":
    ...
    while(len > i){ //probably should await this...
        let cmd = s.getScryptyCommands(scrypty)[i].cmd;
...
function findWorkingDir(programName: string){
    ...
    fs.readdir(folder, function (err: any, files: string | any[]) {
        if (err) {
          console.log(err);
          return;
        }
...
function checkIfExecFails(command: string){
    let fails = true;
    return new Promise((resolve) => {
...
async function gitDownload(url: string, programName: string){
    ...
    exec("git clone " + url + " " + folder + " --recursive", (error: { message: any; }, stdout: any, stderr: any) => {
        if (error) {
            console.log(`error: ${error.message}`);
            s.log(error, 4);
         //   return;
...
async function zipDownload(programName: string, url: string) {
    ...
    request.get(url)
        ...
        .on('finish', async () =>{
            ...
            .pipe(unzipper.Extract({ path: './' + programName }));
             fs.unlink("./" + programName + "/" + url.substr(url.lastIndexOf("/")), (err: any) => {
                if(err){
                    console.error(err);
                    s.log(err, 4);
                    return;
