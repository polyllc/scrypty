...
function download(url) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield s.setLogFile(programName + (new Date().getTime() / 1000));
        s.log("Using Scrypty " + scryptyVersion);
        s.log("Downloading " + url);
        s.log("Program name is " + programName);
        yield checkPrerequisitesPromises().then((results) => {
            let i = 0;
        ...
        if (fileToDownload.endsWith(".git")) {
            yield gitDownload(fileToDownload, programName);
            ...
            yield zipDownload(programName, fileToDownload);
...
function compile(programName, ignoreScrypty = 0) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (validScrypty == 0) {
            ...
            file = yield findMethods(folder, programName, methods); //no idea why, but methods is global, so we don't even need to return that
        }
        //compile
        if (methods.length == 1) {
            console.log(colorText(_black, "Compiling by " + methods[0], _bgWhite));
...
function findMethods(folder, programName, methods) {
    return __awaiter(this, void 0, void 0, function* () {
        let file = new Map();
        ...
        //
        let cmake = yield findIfCMake(folder);
        if (cmake) {
            s.log("Can compile by CMake...");
            methods.push("cmake");
        }
        let vs = yield findVSMethod(folder, programName);
        if (vs) {
            s.log("Can compile by Visual Studio...");
            file.set("vs", vs);
            methods.push("vs");
        }
        let cpp = yield findIfCpp(folder, programName);
        if (cpp) {
...
function findIfCpp(folder, programName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let res = yield getDirectories(__dirname + "\\" + programName);
...
function findVSMethod(folder, programName) {
    return __awaiter(this, void 0, void 0, function* () {
        let sln = "";
        let res = yield getDirectories(__dirname + "\\" + programName); //uhh this returns all files in the folder, which takes a while, but compiling takes longer so the user will have to wait :)
...
function findIfCMake(folder) {
    return __awaiter(this, void 0, void 0, function* () {
        let res = yield getDirectories(folder);
...
function compileByMethod(method, scrypty, folder, programName, file = new Map()) {
    return __awaiter(this, void 0, void 0, function* () {
        switch (method) {
            case "singleg++":
                if (scrypty) {
                    compileSingleGPP(folder, getScryptyOS(scrypty).mainFile, programName);
                    break;
                }
                else {
                    ...
                    compileSingleGPP(folder, file.get("singleg++"), programName);
                    break;
                }
            case "singlegcc":
                if (scrypty) {
                    compileSingleGCC(folder, getScryptyOS(scrypty).mainFile);
                    break;
                }
                else {
                    compileSingleGCC(folder, file.get("singlegcc"));
                    break;
                }
            case "singlego":
                if (scrypty) {
                    compileSingleGo(folder, getScryptyOS(scrypty).mainFile);
                    break;
                }
                else {
                    compileSingleGo(folder, file.get("singlego"));
                    break;
                }
            case "vs":
                if (scrypty) {
                    yield compileVSSolution(folder, programName + "\\" + getScryptyOS(scrypty).vsSolution, programName);
                }
                else {
                    yield compileVSSolution(folder, file.get("vs"), programName);
                }
                break;
            case "cmake":
...
function compileSingleGPP(folder, file, programName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (file == "none" || file === undefined || file == "2") { //if we still have no file selected
            let files = yield getDirectories(__dirname + "\\" + programName);
...
function compileSingleGCC(folder, file) {
    console.log("Compiling file " + file + "...");
...
function compileSingleGo(folder, file) {
    console.log("Compiling file " + file + "...");
    s.log("Compiling file " + file + "...");
...
function compileVSSolution(folder, file, programName) {
    ...
    */
    ...
    fs.readFile(file, 'utf8', (err, data) => {
        ...
        exec("msbuild -t:restore" + sdk + " -p:RestorePackagesConfig=true " + pt + cp + " " + file, { maxBuffer: 1024 * 4000 }, (error, stdout, stderr) => {
            if (stderr) {
                console.log(`${stderr}`);
...
function compileCmake(folder, programName) {
    ...
    switch (option) {
        case "1":
            exec("cd " + folder + " && cmake -S ./ -B ./build", (error, stdout, stderr) => {
                if (error) {
                    console.log(`error: ${error.message}`);
...
function compileCustom(scrypty, programName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (r != ("y" || "n")) {
            ...
            switch (r) {
                case "y":
                    console.log(colorText(_green, "Continuing..."));
                    break;
                case "n":
                ...
                case "c":
                    ...
                    break;
                default:
                    console.log(colorText(_yellow, "Not a valid option"));
                    break;
            }
        }
...
function checkPrerequisitesPromises() {
    return __awaiter(this, void 0, void 0, function* () {
        let promises = [];
...
function gitDownload(url, programName) {
    return __awaiter(this, void 0, void 0, function* () {
        let folder = __dirname + "\\repositories\\" + programName;
        fs.mkdir(folder, () => {
...
function zipDownload(programName, url) {
    return __awaiter(this, void 0, void 0, function* () {
