...
function colorText(color: string, str: string, bg = _bgBlack){
...
//some of these don't change
interface CompileAll extends CompileOS {
    vsSolution: string;
} 
interface CompileWin extends CompileOS {
    vsSolution: string;
} 


interface ScryptyFile{
    info?: {
        name: string;
...
let blankScryptyFile : ScryptyFile; //something has gone really wrong if it gets to here (when its supposed to be using a scrypty that is)




function getMethod(scrypty: ScryptyFile){
    switch(os.platform()){
...
function getScryptyCommands(scrypty: ScryptyFile){
    switch(os.platform()){
...
function getScryptyOS(scrypty: ScryptyFile){ //gets the compile data from scrypty file by os
    switch(os.platform()){
...
function getScryptyFile(scrypty: ScryptyFile){ //gets the compile data from scrypty file by os
    switch(os.platform()){
...
function verifyScrypty(scryptyFile: ScryptyFile){ //so much verifying to do!
...
function findIfScrypty(files: any[]) {
...
function parseScrypty(file: any){
...
function log(message: string, severity = 1){
    let file = getLogFile();
...
function logBoth(message: string, severity = 1){
    console.log(message);
